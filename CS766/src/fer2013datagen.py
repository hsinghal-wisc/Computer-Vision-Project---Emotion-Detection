...
def load_data(sample_split=0.3, usage='Training', to_cat=True, verbose=True,
    ...
    rows = random.sample(data.index, sample_split)
    data = data.ix[rows]
...
if __name__ == '__main__':
    ...
    emo = ['Angry', 'Fear', 'Happy',
           'Sad', 'Surprise', 'Neutral']
    X_train, y_train, emo_dict = load_data(sample_split=3589,
                                           classes=emo,
                                           usage='PrivateTest',
                                           verbose=True)
    print 'Saving...'
    save_data(X_train, y_train, fname='_private')
    print X_train.shape
    print y_train.shape
    ...
    X_train, y_train, emo_dict = load_data(sample_split=3589,
                                           ...
                                           verbose=True)
    print 'Saving...'
    save_data(X_train, y_train, fname='_public')
    print X_train.shape
    print y_train.shape
                                           
    X_train, y_train, emo_dict = load_data(sample_split=28709,
                                            classes=emo,
                                            usage='Training',
                                            verbose=True)
    print 'Saving...'
