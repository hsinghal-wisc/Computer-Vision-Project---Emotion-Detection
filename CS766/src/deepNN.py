...
def cnn_architecture(X_train, y_train, conv_arch=[(32,3),(64,3),(128,3)],
    ...
    describeGraph(X_shape, y_shape, batch_size, dropout, nb_epoch, conv_arch, dense)
    
    
    X_train, X_test, y_train, y_test = train_test_split(X_train, y_train, test_size=validation_split)
    datagen = ImageDataGenerator( rotation_range=10, shear_range=0.2, width_shift_range=0.2,
...
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    ...
    Logstats(model, starttime, batch_size, nb_epoch, conv_arch, dense,
...
def Logstats(model, starttime, batch_size, nb_epoch, conv_arch,dense, dropout,
...
def describeGraph(X_shape, y_shape, batch_size, dropout, nb_epoch, conv_arch, dense):
...
if __name__ == '__main__':
    ...
    cnn_architecture(X_train, y_train, conv_arch=[(32,3),(64,3),(128,3)], dense=[64,2], batch_size=256, nb_epoch=50, dirpath = '../data/results/')
